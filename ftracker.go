...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInMsec := kmhInMsec * meanSpeed(action, duration) // средняя скорость в м/с
	heightInM := height / cmInM                                // рост в метрах
	spentCalories := (walkingCaloriesWeightMultiplier*weight + math.Pow(meanSpeedInMsec, 2)/heightInM*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return spentCalories
}
